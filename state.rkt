...
(struct state (name function type posn) #:mutable)
...
(define (state-pressed? mouse-x mouse-y tbox)
  (cond
    [(and (and (> mouse-x (- (posn-x (state-posn tbox)) (/ (state-posn tbox) 2)))
               (< mouse-x (+ (posn-x (state-posn tbox)) (/ (state-posn tbox) 2))))
          (and (> mouse-y (- (posn-y (state-posn tbox)) (/ (state-posn tbox) 2)))
               (< mouse-y (+ (posn-y (state-posn tbox)) (/ (state-posn tbox) 2)))))
     #t]
    [else #f]))